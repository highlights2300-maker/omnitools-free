    ...
    <div>
      ...
      <div className="flex flex-col gap-6 lg:grid lg:grid-cols-[1fr_360px]">
        <div className="no-print flex flex-col gap-5">
          <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            ...
            <div className="flex flex-col gap-3 md:grid md:grid-cols-2">
              ...
              <input type="tel" inputMode="tel" className={`${inputClass} md:col-span-2`} placeholder="Phone" value={business.phone} onChange={(e) => setBusiness({ ...business, phone: e.target.value })} />
            </div>
          </section>

          <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-500">Received from</h2>
            <div className="flex flex-col gap-3 md:grid md:grid-cols-2">
              <input className={inputClass} placeholder="Payer name" value={payer.name} onChange={(e) => setPayer({ ...payer, name: e.target.value })} />
              <input type="email" className={inputClass} placeholder="Payer email" value={payer.email} onChange={(e) => setPayer({ ...payer, email: e.target.value })} />
            </div>
          </section>

          <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-500">Receipt details</h2>
            <div className="flex flex-col gap-3 md:grid md:grid-cols-4">
              <input className={inputClass} placeholder="Receipt #" value={meta.number} onChange={(e) => setMeta({ ...meta, number: e.target.value })} />
              <input type="date" className={inputClass} value={meta.date} onChange={(e) => setMeta({ ...meta, date: e.target.value })} />
              <select className={inputClass} value={meta.method} onChange={(e) => setMeta({ ...meta, method: e.target.value })}>
          ...
          <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-xs font-semibold uppercase tracking-wider text-slate-500">What was paid for</h2>
              ...
              {items.map((it) => (
                <div key={it.id} className="flex flex-col gap-2 rounded-lg border border-slate-800 p-3 md:grid md:grid-cols-[1fr_110px_40px] md:items-center md:gap-2 md:border-0 md:p-0">
                  ...
                  <div className="flex gap-2 md:contents">
                    <input type="number" inputMode="decimal" min="0" step="0.01" className={`${inputClass} text-right font-mono`} value={it.amount} onChange={(e) => updateItem(it.id, "amount", e.target.value)} />
                    <button onClick={() => removeItem(it.id)} aria-label="Remove line" className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg text-slate-600 transition hover:bg-red-500/10 hover:text-red-400 md:h-12 md:w-10">
        ...
        <div className="lg:sticky lg:top-6 lg:self-start">
          <div className="receipt-print-area rounded-2xl border border-slate-800 bg-white p-6 text-slate-900 shadow-2xl sm:p-8">
