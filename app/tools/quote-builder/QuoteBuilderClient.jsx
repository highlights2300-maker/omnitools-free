    ...
    <div>
      ...
      <div className="flex flex-col gap-6 lg:grid lg:grid-cols-[1fr_380px]">
        <div className="no-print flex flex-col gap-5">
          <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            ...
            <div className="flex flex-col gap-3 md:grid md:grid-cols-2">
              <input className={inputClass} placeholder="Business name" value={business.name} onChange={(e) => setBusiness({ ...business, name: e.target.value })} />
              <input type="email" className={inputClass} placeholder="Email" value={business.email} onChange={(e) => setBusiness({ ...business, email: e.target.value })} />
              <input type="tel" inputMode="tel" className={inputClass} placeholder="Phone" value={business.phone} onChange={(e) => setBusiness({ ...business, phone: e.target.value })} />
              <textarea className={`${textareaClass} md:col-span-2`} rows={2} placeholder="Address" value={business.address} onChange={(e) => setBusiness({ ...business, address: e.target.value })} />
            </div>
          </section>

          <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-500">Prepared for</h2>
            <div className="flex flex-col gap-3 md:grid md:grid-cols-2">
              <input className={inputClass} placeholder="Client name" value={client.name} onChange={(e) => setClient({ ...client, name: e.target.value })} />
              <input type="email" className={inputClass} placeholder="Client email" value={client.email} onChange={(e) => setClient({ ...client, email: e.target.value })} />
              <textarea className={`${textareaClass} md:col-span-2`} rows={2} placeholder="Client address" value={client.address} onChange={(e) => setClient({ ...client, address: e.target.value })} />
            </div>
          </section>

          <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-500">Quote details</h2>
            <div className="flex flex-col gap-3 md:grid md:grid-cols-4">
              <input className={inputClass} placeholder="Quote #" value={meta.number} onChange={(e) => setMeta({ ...meta, number: e.target.value })} />
              <input type="date" className={inputClass} value={meta.date} onChange={(e) => setMeta({ ...meta, date: e.target.value })} />
              <input type="date" className={inputClass} value={meta.validUntil} onChange={(e) => setMeta({ ...meta, validUntil: e.target.value })} />
          ...
          <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={addItem} className="inline-flex h-10 items-center gap-1.5 rounded-lg border border-slate-700 px-3 text-xs font-medium text-slate-300 transition hover:bg-slate-800">
            ...
            <div className="flex flex-col gap-3">
              {items.map((it) => (
                <div key={it.id} className="flex flex-col gap-2 rounded-lg border border-slate-800 p-3 md:grid md:grid-cols-[1fr_80px_110px_40px] md:items-center md:gap-2 md:border-0 md:p-0">
                  <input className={inputClass} placeholder="Description" value={it.desc} onChange={(e) => updateItem(it.id, "desc", e.target.value)} />
                  <div className="flex gap-2 md:contents">
                    <input type="number" inputMode="decimal" min="0" className={`${inputClass} text-center`} value={it.qty} onChange={(e) => updateItem(it.id, "qty", e.target.value)} />
                    <input type="number" inputMode="decimal" min="0" step="0.01" className={`${inputClass} text-right font-mono`} value={it.rate} onChange={(e) => updateItem(it.id, "rate", e.target.value)} />
                    <button onClick={() => removeItem(it.id)} aria-label="Remove item" className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg text-slate-600 transition hover:bg-red-500/10 hover:text-red-400 md:h-12 md:w-10">
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <input type="number" inputMode="decimal" min="0" step="0.1" className={`${inputClass} w-24 font-mono`} value={taxRate} onChange={(e) => setTaxRate(e.target.value)} />
            </div>
          </section>

          <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-500">Terms & notes</h2>
        ...
        <div className="lg:sticky lg:top-6 lg:self-start">
          <div className="quote-print-area rounded-2xl border border-slate-800 bg-white p-6 text-slate-900 shadow-2xl sm:p-8">
